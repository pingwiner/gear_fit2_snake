...
void draw_text(appdata_s *ad) {
	...
	cairo_set_source_rgba(ad->cairo, 1.0, 1.0, 1.0, 0.8);
...
void draw_game_over(appdata_s *ad) {
	const char* buf = "GAME OVER";
	cairo_text_extents_t extents;
	double x, y;
	cairo_set_source_rgba(ad->cairo, 1.0, 0.0, 0.0, 0.8);
 	cairo_select_font_face(ad->cairo, "Sans",
		CAIRO_FONT_SLANT_NORMAL,
	...
	cairo_text_extents(ad->cairo, buf, &extents);
	x = ad->width / 2 - (extents.width / 2 + extents.x_bearing);
	y = ad->height / 2 - (extents.height / 2 + extents.y_bearing);
	cairo_move_to(ad->cairo, x, y);
...
void cairo_drawing(void *data, Evas_Object *obj)
{
	...
	for(i = 0; i < FIELD_SIZE_Y; i++)
	{
		for(j = 0; j < FIELD_SIZE_X; j++) {
			...
			float x = j * cell_size;
			float y = i * cell_size;
			cairo_rectangle(ad->cairo, x + padding, y, cell_size - 1, cell_size - 1);
			cairo_fill(ad->cairo);
		}
	}
	draw_text(ad);
...
app_create(void *data)
{
	...
	model_init();
	init_palette();
	cell_size = SCREEN_SIZE_X / FIELD_SIZE_X;
	padding = (SCREEN_SIZE_X % FIELD_SIZE_X) / 2;
